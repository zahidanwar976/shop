    ...
    <table class="bs-0 mb-30 px-10">
        <tr>
            <th class="content-position-y text-left">
                ...
                <h4 class="text-uppercase mb-1 fz-14">
                    {{translate('Shop_Name')}}
                    : {{ $order->seller_is == 'admin' ? $company_name : (isset($order->seller->shop) ? $order->seller->shop->name : translate('not_found')) }}
                </h4>
                @if($order['seller_is']!='admin' && isset($order['seller']->gst) != null)
                    <h4 class="text-capitalize fz-12">{{translate('GST')}}
                        : {{ $order['seller']->gst }}</h4>
                @endif
            </th>
            <th class="content-position-y text-right">
                <h4 class="fz-14">{{translate('date')}} : {{date('d-m-Y h:i:s a',strtotime($order['created_at']))}}</h4>
            </th>
        </tr>
    </table>
</div>
                                ...
                                <td>
                                    @if ($order->shippingAddress)
                                        ...
                                        <div class="h4 montserrat-normal-600">
                                            <p class="font-weight-normal" style=" margin-top: 6px; margin-bottom:0px;">{{$order->customer !=null? $order->customer['f_name'].' '.$order->customer['l_name']:translate('name_not_found')}}</p>
                                            <p class="font-weight-normal" style=" margin-top: 6px; margin-bottom:0px;">{{$order->customer !=null? $order->customer['email']:translate('email_not_found')}}</p>
                                            <p class="font-weight-normal" style=" margin-top: 6px; margin-bottom:0px;">{{$order->customer !=null? $order->customer['phone']:translate('phone_not_found')}}</p>
                                    ...
                                    @else
                                        <span class="h2" style="margin: 0px;">{{translate('customer_info')}} </span>
                                        <div class="h4 montserrat-normal-600">
                                            <p class="font-weight-normal" style=" margin-top: 6px; margin-bottom:0px;">{{$order->customer !=null? $order->customer['f_name'].' '.$order->customer['l_name']:translate('name_not_found')}}</p>
                                            @if (isset($order->customer) && $order->customer['id']!=0)
                                                <p style=" margin-top: 6px; margin-bottom:0px;">{{$order->customer !=null? $order->customer['email']:translate('email_not_found')}}</p>
                                                <p style=" margin-top: 6px; margin-bottom:0px;">{{$order->customer !=null? $order->customer['phone']:translate('phone_not_found')}}</p>
                                            @endif
                                        </div>
                                        @endif
                                        </p>
            ...
            <table class="content-position-y" style="width: 100%">
                <tr>
                    <td class="text-center" valign="top">
                        <span class="h2" style="margin: 0px;">{{translate('POS_order')}} </span>
    ...
    <table class="fz-12">
        <tr>
            <th class="text-left" style="width: 60%">
                <h4 class="fz-12 mb-1">{{translate('payment_details')}}</h4>
                ...
                @if ($order->delivery_type !=null)
                    <h4 class="fz-12 mb-1">{{translate('delivery_info')}} </h4>
