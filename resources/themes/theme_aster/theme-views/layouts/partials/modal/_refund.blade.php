            ...
            <div class="modal-body px-sm-5">
                    <div class="d-flex flex-column flex-sm-row flex-wrap gap-4 justify-content-between mb-4">
                        ...
                        <div class="d-flex flex-column gap-1 fs-12">
                            <span>{{('Qty')}} : {{$order_details->qty}}</span>
                            <span>{{('Price')}} : {{\App\CPU\Helpers::currency_converter($order_details->price)}}</span>
                            <span>{{('Discount')}} : {{\App\CPU\Helpers::currency_converter($order_details->discount)}}</span>
                            <span>{{('Tax')}} : {{\App\CPU\Helpers::currency_converter($order_details->tax)}}</span>
                        </div>
                ...
                <form action="{{route('refund-store')}}"  method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group mb-4">
                        ...
                        <textarea name="refund_reason"  class="form-control" rows="4" placeholder="{{  translate('Refund_reason')}}"required></textarea>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="d-flex flex-column gap-3">
                            ...
                            <div class="text-muted">{{translate('File_type:_jpg_jpeg_png._Maximum_size')}}: {{translate('2MB')}}</div>
                        </div>
                    </div>
                    <div class="modal-footer mb-4 gap-3 px-sm-5 pb-4">
                        <button type="button" class="btn btn-secondary m-0" data-bs-dismiss="modal">{{translate('Cancel')}}</button>
